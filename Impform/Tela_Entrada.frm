    ...
    Else
        ...
        For I = 0 To (LT_Menus.ListIndex - 1)
            If LT_Menus.Selected(I) = True Then
                CVM Trim(LT_Menus.List(I)), True
            Else
                CVM Trim(LT_Menus.List(I)), False
            End If
        Next I
        With Tela_Siscoval
            .BF.Buttons(4).Enabled = .Menu_Principal_Cotacoes.Enabled 'BT Cotação
            .BF.Buttons(5).Enabled = .Menu_Principal_Pedidos.Enabled 'BT Pedidos
        End With
    End If
