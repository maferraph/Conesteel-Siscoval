...
' ****************** DECLARAÇÕES ****************
Const NOMEAPLIC As String = "Consulta à Fichas de Estoque"
Dim RespMsg
Dim I As Integer
...
Private Sub BT_Procurar_Click()
    ...
    ElseIf RB_Data.Value = True And TXT_Data.Text = "__/__/____" Then
        RespMsg = MsgBox("Não foi selecionada uma data para procura.", vbOKOnly, NOMEAPLIC)
        TXT_Data.SetFocus
        Exit Sub
    ElseIf RB_NF.Value = True And TXT_NF.Text = "" Then
        RespMsg = MsgBox("Não foi selecionado um número de nota fiscal para procura.", vbOKOnly, NOMEAPLIC)
        TXT_NF.SetFocus
        Exit Sub
    End If
